...
class BrainGPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config['encoder_config']['bidirectional']:
            self.linear = nn.Linear(
                ...
                LLM_decoder[config['decoder_config']
                            ['name']]['hidden_size']).to(self.device)
        else:
            self.linear = nn.Linear(
        ...
        self.llm = LLM_decoder[
            config['decoder_config']['name']]['model'].from_pretrained(
                LLM_decoder[config['decoder_config']['name']]['path'],
    ...
    def forward(self, spikepow, labels=None, key=None, pad_token_id=1):
        ...
        spikepow_embeds = self.encoder(spikepow,
                                       key=key).transpose(0, 1)  #B * L * F
        ...
        spikepow_ids = (-100 * torch.ones(
            (spikepow_embeds.shape[0], spikepow_embeds.shape[1]))).to(
                self.device)
        ...
        inputs_embeds = torch.cat((spikepow_embeds, tgt_embeds),
                                  dim=1)  # B * L * F
    ...
    def generate(self, spikepow, key, tokenizer):
        ...
        batch_size, seq_len, feature_len = spikepow_embeds.shape[
            0], spikepow_embeds.shape[1], spikepow_embeds.shape[2]

        start_token_id = torch.ones(
            (batch_size, 1)).to(self.device) * tokenizer.bos_token_id
        if 'gpt2' in self.config['decoder_config']['name']:
            start_token_embed = self.llm.transformer.wte(start_token_id.int())
        elif 'opt' in self.config['decoder_config']['name']:
            start_token_embed = self.llm.model.decoder.embed_tokens(
                start_token_id.int())
        elif 'llama' in self.config['decoder_config']['name']:
            start_token_embed = self.llm.model.embed_tokens(
                start_token_id.int())
        spikepow_embeds = torch.cat((spikepow_embeds, start_token_embed),
                                    dim=1)
    ...
    def load_encoder_checkpoint(self, path):
        self.linear.load_state_dict(
            torch.load(path + '/linear.pt', map_location=self.device))
    ...
    for key in eval_dataloaders.keys():
        ...
        for src, tgt, src_length, tgt_length in eval_dataloader:
            ...
            for i in range(outputs.shape[0]):
                ...
                hw = outputs[i, :]
                hw = hw[(hw != tokenizer.eos_token_id)
                        & (hw != tokenizer.pad_token_id)]
                rw = tgt[i, :]
                rw = rw[(rw != tokenizer.eos_token_id)
                        & (rw != tokenizer.pad_token_id) & (rw != -100)]
                hw = tokenizer.decode(hw.int())
                rw = tokenizer.decode(rw.int())

                hw = hw.lower().replace('.', '').replace(',', '').replace(
                    '?', '').replace('-', ' ').replace(';', '').replace(
                        '!', '').replace(':', '').replace('"', '')
                rw = rw.lower().replace('.', '').replace(',', '').replace(
                    '?', '').replace('-', ' ').replace(';', '').replace(
                        '!', '').replace(':', '').replace('"', '')
...
if __name__ == '__main__':
    ...
    tokenizer = LLM_decoder[llm_type]['tokenizer'].from_pretrained(
        LLM_decoder[llm_type]['path'])
    train_dataloaders, val_dataloaders, eval_train_dataloaders, eval_val_dataloaders = prepare_dataloader(
        data_config['data_path'], sessions, tokenizer, config)
    ...
    param_list.append({
        ...
        'lr': train_config['lr_linear']
    })
    param_list.append({
        'params': model.encoder.parameters(),
        'lr': train_config['lr_encoder']
    })
    ...
    scheduler = CosineAnnealingLR(optimizer,
                                  T_max=num_steps * train_config['num_epochs'])
    ...
    count = 0
    stop_flag = False
    checkpoint_path = train_config['checkpoint_path'] + '/' + ''.join([
        str(k) + ':' + str(v) + '|'
            ...
            if val_mean_wer < best_val_wer:
                ...
                if train_config['early_stopping']:
                    if count > train_config['patience']:
                        stop_flag = True
            print(
                f"Epoch: {epoch},Best val wer: {best_val_wer:.3f},Current val wer: {val_mean_wer:.3f} "
