...
config_path = '/burg-archive/home/zc2666/b2t-project/BrainLLM/config/config.yaml'
...
if encoder_config['name'] == 'BGRU':
    ...
    test_bpes = []
    with open(
            '/GPFS/data/shengfeng-1/BrainGPT_data/derived/new_transcriptions/train_bpe.txt',
            'r') as f:
        for line in f.readlines():
            train_bpes.append([int(s) for s in line.strip('\n').split(' ')])
    with open(
            '/GPFS/data/shengfeng-1/BrainGPT_data/derived/new_transcriptions/test_bpe.txt',
            'r') as f:
        for line in f.readlines():
...
for session in tqdm(sessions):
    ...
    index_path = None
    tmp_train_dataset = TFRecordDataset(
        tfrecord_path + '/train/chunk_0.tfrecord', index_path)
    tmp_train_loader = torch.utils.data.DataLoader(tmp_train_dataset,
                                                   batch_size=1)
    for data in tmp_train_loader:
        input_seq = data['inputFeatures'].reshape(
            -1, encoder_config['dim_feature'])
        if encoder_config['name'] == 'BGRU':
    ...
    tmp_val_dataset = TFRecordDataset(tfrecord_path + '/test/chunk_0.tfrecord',
                                      index_path)
...
gaussian_kernel = torch.Tensor([
    np.exp(-(x**2) / (2 * data_config['sigma']**2))
    for x in range(-gaussain_kernel_size, gaussain_kernel_size + 1)
...
def collate_fn_stochastic(batch):
    ...
    if data_config['white_noise_sd'] > 0:
        src_batch += np.sqrt(data_config['white_noise_sd']) * torch.randn(
            src_batch.shape[0], src_batch.shape[1], src_batch.shape[2])
    if data_config['constant_offset_sd'] > 0:
        src_batch += np.sqrt(data_config['constant_offset_sd']) * torch.randn(
            src_batch.shape[0], 1, src_batch.shape[-1])
    src_batch = torch.Tensor(src_batch).float()
    if 'use_random_mask' in encoder_config.keys(
    ) and encoder_config['use_random_mask']:
        src_batch = apply_random_mask(src_batch,
                                      encoder_config['mask_bandwidth'])
    if 'use_sign_flip' in encoder_config.keys(
    ) and encoder_config['use_sign_flip']:
        src_batch = apply_sign_flip(src_batch,
                                    encoder_config['sign_flip_proba'])
    if 'use_time_reverse' in encoder_config.keys(
    ) and encoder_config['use_time_reverse']:
        src_batch = apply_time_reverse(src_batch,
                                       encoder_config['time_reverse_proba'])
    src_batch = F.conv2d(src_batch.unsqueeze(1).transpose(0, -1),
                         gaussian_kernel,
                         padding='same').squeeze(1).transpose(0, -1)
    #print(src_batch.shape) # L*B*F
    return src_batch, torch.Tensor(tgt_batch), torch.Tensor(src_length).to(
        torch.int32), torch.Tensor(tgt_length).to(torch.int32)
...
def collate_fn(batch):
    ...
    src_batch = F.conv2d(src_batch.unsqueeze(1).transpose(0, -1),
                         gaussian_kernel,
                         padding='same').squeeze(1).transpose(0, -1)
    return torch.Tensor(src_batch), torch.Tensor(tgt_batch), torch.Tensor(
        src_length).to(torch.int32), torch.Tensor(tgt_length).to(torch.int32)
...
for key in train_datasets.keys():
    train_dataloaders[key] = DataLoader(
        ...
        collate_fn=collate_fn_stochastic,
        shuffle=True)
for key in val_datasets.keys():
    val_dataloaders[key] = DataLoader(val_datasets[key],
                                      batch_size=pretrain_config['batch_size'],
                                      collate_fn=collate_fn_stochastic,
                                      shuffle=True)

for key in train_datasets.keys():
    eval_train_dataloaders[key] = DataLoader(
        train_datasets[key],
        batch_size=pretrain_config['batch_size'],
        collate_fn=collate_fn)
for key in val_datasets.keys():
    eval_val_dataloaders[key] = DataLoader(
        val_datasets[key],
        batch_size=pretrain_config['batch_size'],
        collate_fn=collate_fn)
...
optimizer = torch.optim.Adam([{
    ...
    'params': model.inputNetworks.parameters(),
    'weight_decay': 0
}],
                             lr=pretrain_config['lr_encoder'],
                             betas=(0.9, 0.999),
                             eps=pretrain_config['lr_encoder'],
                             weight_decay=pretrain_config['l2_coef'])
num_steps = 0
...
scheduler = get_linear_schedule_with_warmup(
    ...
    num_training_steps=num_steps * pretrain_config['num_epochs'])
...
count = 0
stop_flag = False
checkpoint_path = pretrain_config['checkpoint_path'] + '/' + ''.join([
    str(k) + ':' + str(v) + '|'
...
for epoch in range(1, pretrain_config['num_epochs'] + 1):
    ...
    if (epoch) % EPOCHS_PER_EVAL == 0:
        ...
        end_time = timer()
        print(f"Epoch: {epoch},Val per: {val_per:.3f} "
              f"evaluation time = {(end_time - start_time):.3f}s")
        if best_per > val_per:
            best_per = val_per
